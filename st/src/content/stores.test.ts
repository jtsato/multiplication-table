...
describe("store content", () => {
  it("defines four stores with three initial and three unlockable products", () => {
    ...
    for (const store of STORES) {
      ...
      expect(store.products.every((product) => product.price >= 1 && product.price <= 10)).toBe(true);
    }
  });

  it("has a recurring cast with distinct customer identities", () => {
    expect(CUSTOMERS.map((customer) => customer.name)).toEqual(["Lia", "Caio", "Bia", "Theo", "Nina", "Davi"]);
