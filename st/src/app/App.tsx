...
export function App({ repository }: AppProps) {
  ...
  return (
    <div className={`app-shell ${activeProfile?.accessibility.largeText ? "large-text" : ""} ${activeProfile?.accessibility.highContrast ? "high-contrast" : ""}`}>
      ...
      {screen === "shop" && activeProfile && <ShopScreen strings={strings} profile={activeProfile} notice={notice} onBack={() => setScreen("store")} onPurchase={purchaseProductById} onCosmeticPurchase={purchaseCosmetic} />}
...
function GameScreen({ strings, profile, session, onStartQuestion, onSelectQuantity, onAnswer, onRetry, onContinue, onLeave, onFinish }: { strings: LocaleBundle; profile: PlayerProfile; session: DaySession; onStartQuestion: () => void; onSelectQuantity: (quantity: number) => void; onAnswer: (value: number) => void; onRetry: () => void; onContinue: () => void; onLeave: () => void; onFinish: () => void }) {
  ...
  }, [profile.accessibility.reducedMotion, session.errorsForCurrent, session.feedback?.kind, session.phase]);
  ...
  return (
    <main id="main-content" tabIndex={-1} className="game-screen service-screen">
      <header className="service-header"><button type="button" className="back-button secondary-button" aria-label={strings.backToShopLabel} onClick={onLeave}>{strings.backToShop}</button><span>{strings.customerCounter(session.completedVisits + 1, session.visits.length)}</span><span className="header-money"><span className="header-money-item"><small>{strings.cashLabel}</small><strong>{strings.money(profile.cash)}</strong></span><span className="header-money-item"><small>{strings.todayLabel}</small><strong>{strings.money(session.revenue)}</strong></span></span></header>
...
function ShopScreen({ strings, profile, notice, onBack, onPurchase, onCosmeticPurchase }: { strings: LocaleBundle; profile: PlayerProfile; notice: string; onBack: () => void; onPurchase: (productId: string, cost: number) => void; onCosmeticPurchase: (cosmeticId: string, cost: number) => void }) {
  const store = getStore(profile.store.storeId);
  ...
  return <main id="main-content" tabIndex={-1} className="form-screen"><button type="button" className="back-button secondary-button" aria-label={strings.backToShopLabel} onClick={onBack}>{strings.backToShop}</button><header className="topbar"><div><p className="eyebrow">{strings.catalog}</p><h1>{strings.productsNew}</h1></div><div className="cash-badge" aria-label={strings.cashBadgeLabel(profile.cash)}>{strings.money(profile.cash)}</div></header>{notice && <div className="notice" role="status" aria-live="polite">{notice}</div>}<p className="muted">{strings.catalogDescription}</p><div className="product-grid">{store.products.map((product) => { const unlocked = profile.store.unlockedProducts.includes(product.id); const cost = product.unlockCost ?? 80; const productName = strings.productName(product); const purchase = purchaseStatus(cost, productName); const hintId = `purchase-hint-${product.id}`; return <article className={`product-card ${unlocked ? "unlocked" : ""}`} key={product.id}><ProductArt id={product.id} size="medium" platform platformColor={store.color} /><h2>{productName}</h2><p>{strings.priceLine(product.price)}</p>{unlocked ? <strong>{strings.available}</strong> : <>{!purchase.canAfford && <span id={hintId} className="sr-only">{purchase.description}</span>}<button type="button" className={`secondary-button purchase-button ${purchase.canAfford ? "purchase-button--available" : "purchase-button--unavailable"}`} aria-disabled={purchase.canAfford ? undefined : true} aria-describedby={purchase.canAfford ? undefined : hintId} onClick={() => onPurchase(product.id, cost)}>{strings.buyForLabel(cost)}</button></>}</article>; })}</div><h2 className="section-title">{strings.decor}</h2><div className="product-grid">{COSMETICS.map((cosmetic) => { const owned = profile.store.cosmetics.includes(cosmetic.id); const productName = strings.cosmeticName(cosmetic.id, cosmetic.name); const purchase = purchaseStatus(cosmetic.cost, productName); const hintId = `purchase-hint-${cosmetic.id}`; return <article className="product-card" key={cosmetic.id}><ProductArt id={cosmetic.id} size="medium" platform /><h2>{productName}</h2><p>{strings.decorDescription}</p>{owned ? <strong>{strings.inCollection}</strong> : <>{!purchase.canAfford && <span id={hintId} className="sr-only">{purchase.description}</span>}<button type="button" className={`secondary-button purchase-button ${purchase.canAfford ? "purchase-button--available" : "purchase-button--unavailable"}`} aria-disabled={purchase.canAfford ? undefined : true} aria-describedby={purchase.canAfford ? undefined : hintId} onClick={() => onCosmeticPurchase(cosmetic.id, cosmetic.cost)}>{strings.buyForLabel(cosmetic.cost)}</button></>}</article>; })}</div></main>;
}
...
function SettingsScreen({ strings, profile, onBack, onSave, onReset }: { strings: LocaleBundle; profile: PlayerProfile; onBack: () => void; onSave: (settings: AccessibilitySettings, audio: AudioSettings) => void; onReset: () => Promise<void> }) {
  ...
  return <main id="main-content" tabIndex={-1} className="form-screen"><button type="button" className="back-button secondary-button" aria-label={strings.backToShopLabel} onClick={onBack}>{strings.backToShop}</button><p className="eyebrow">{strings.settingsEyebrow}</p><h1>{strings.settings}</h1><fieldset><legend>{strings.accessibility}</legend><label className="check-row"><input type="checkbox" checked={settings.reducedMotion} onChange={(event) => setSettings({ ...settings, reducedMotion: event.target.checked })} /> {strings.reduceMotion}</label><label className="check-row"><input type="checkbox" checked={settings.largeText} onChange={(event) => setSettings({ ...settings, largeText: event.target.checked })} /> {strings.largeText}</label><label className="check-row"><input type="checkbox" checked={settings.highContrast} onChange={(event) => setSettings({ ...settings, highContrast: event.target.checked })} /> {strings.highContrast}</label></fieldset><fieldset><legend>{strings.audioAndNarration}</legend><label className="check-row"><input type="checkbox" checked={audio.effects} onChange={(event) => setAudio({ ...audio, effects: event.target.checked })} /> {strings.audioEffects}</label><label className="check-row"><input type="checkbox" checked={audio.narration} onChange={(event) => setAudio({ ...audio, narration: event.target.checked })} /> {strings.narration}</label></fieldset><button type="button" className="primary-button" onClick={() => onSave(settings, audio)}>{strings.saveSettings}</button><section className="danger-zone"><h2>{strings.resetTitle}</h2><p className="muted">{strings.resetDescription}</p>{confirmingReset
