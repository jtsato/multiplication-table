...
function ShopScreen({ strings, profile, onBack, onPurchase, onCosmeticPurchase }: { strings: LocaleBundle; profile: PlayerProfile; onBack: () => void; onPurchase: (productId: string, cost: number) => void; onCosmeticPurchase: (cosmeticId: string, cost: number) => void }) {
  const store = getStore(profile.store.storeId);
  const purchaseStatus = (cost: number) => {
    const canAfford = profile.cash >= cost;
    return {
      canAfford,
      label: canAfford ? strings.purchaseReady : strings.purchaseNeedsMore(strings.money(cost - profile.cash)),
    };
  };
  return <main id="main-content" tabIndex={-1} className="form-screen"><button type="button" className="back-button secondary-button" aria-label={strings.backToShopLabel} onClick={onBack}>{strings.backToShop}</button><header className="topbar"><div><p className="eyebrow">{strings.catalog}</p><h1>{strings.productsNew}</h1></div><div className="cash-badge" aria-label={strings.cashBadgeLabel(profile.cash)}>{strings.money(profile.cash)}</div></header><p className="muted">{strings.catalogDescription}</p><div className="product-grid">{store.products.map((product) => { const unlocked = profile.store.unlockedProducts.includes(product.id); const cost = product.unlockCost ?? 80; const purchase = purchaseStatus(cost); return <article className={`product-card ${unlocked ? "unlocked" : ""}`} key={product.id}><ProductArt id={product.id} size="medium" platform platformColor={store.color} /><h2>{strings.productName(product)}</h2><p>{strings.priceLine(product.price)}</p>{unlocked ? <strong>{strings.available}</strong> : <><p className={`purchase-state ${purchase.canAfford ? "purchase-state--can" : "purchase-state--cannot"}`}><span aria-hidden="true">{purchase.canAfford ? "✓" : "!"}</span>{purchase.label}</p><button type="button" className="secondary-button" disabled={!purchase.canAfford} onClick={() => onPurchase(product.id, cost)}>{strings.buyForLabel(cost)}</button></>}</article>; })}</div><h2 className="section-title">{strings.decor}</h2><div className="product-grid">{COSMETICS.map((cosmetic) => { const owned = profile.store.cosmetics.includes(cosmetic.id); const purchase = purchaseStatus(cosmetic.cost); return <article className="product-card" key={cosmetic.id}><ProductArt id={cosmetic.id} size="medium" platform /><h2>{strings.cosmeticName(cosmetic.id, cosmetic.name)}</h2><p>{strings.decorDescription}</p>{owned ? <strong>{strings.inCollection}</strong> : <><p className={`purchase-state ${purchase.canAfford ? "purchase-state--can" : "purchase-state--cannot"}`}><span aria-hidden="true">{purchase.canAfford ? "✓" : "!"}</span>{purchase.label}</p><button type="button" className="secondary-button" disabled={!purchase.canAfford} onClick={() => onCosmeticPurchase(cosmetic.id, cosmetic.cost)}>{strings.buyForLabel(cosmetic.cost)}</button></>}</article>; })}</div></main>;
}
