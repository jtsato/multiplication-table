...
export function App({ repository }: AppProps) {
  ...
  function startDay(): void {
    ...
    setSession(
      createDaySession(
        ...
        activeProfile.day,
        daySeed,
        activeProfile.store.unlockedProducts,
        Object.values(activeProfile.mathProgress),
      ),
    );
    setNotice("");
    setScreen("game");
  ...
  function finishDay(): void {
    ...
    const completedObjectives =
      session.completedVisits >= objective.requiredVisits &&
      !activeProfile.objectives.completed.includes(objective.id)
  ...
  return (
    <div
      className={`app-shell ${activeProfile?.accessibility.largeText ? "large-text" : ""} ${activeProfile?.accessibility.highContrast ? "high-contrast" : ""}`}
    >
      ...
      {screen === "game" && activeProfile && session && (
        <GameScreen
          strings={strings}
          profile={activeProfile}
          session={session}
          onStartQuestion={() =>
            setSession((current) => (current ? { ...current, phase: "question" } : current))
          }
          onSelectQuantity={(quantity) =>
            setSession((current) => (current ? selectQuantity(current, quantity) : current))
          }
          onAnswer={answerQuestion}
          onRetry={() => setSession((current) => (current ? retryQuestion(current) : current))}
          onContinue={() =>
            setSession((current) => (current ? continueAfterFeedback(current) : current))
          }
          onLeave={() => setScreen("store")}
          onFinish={finishDay}
        />
      )}
      {screen === "shop" && activeProfile && (
        <ShopScreen
          ...
          onPurchase={purchaseProductById}
          onCosmeticPurchase={purchaseCosmetic}
        />
      )}
      {screen === "achievements" && activeProfile && (
        <AchievementsScreen
          strings={strings}
          profile={activeProfile}
          onBack={() => setScreen("store")}
      ...
      <form
        onSubmit={(event) => {
          ...
          void onCreate({
            ...
            style,
            mascot,
            accessibility: { reducedMotion, largeText },
          });
        }}
      >
        ...
        <fieldset>
          ...
          <div className="store-choice-grid">
            {STORES.map((store) => {
              const text = strings.storeText(store);
              return (
                <button
                  type="button"
                  className={`store-choice ${store.id === storeId ? "selected" : ""}`}
                  key={store.id}
        ...
        <fieldset>
          <legend>{strings.chooseStyle}</legend>
          <div className="style-choice-grid">
            {(
              [
                { id: "sunrise", color: "#e57a44" },
                { id: "ocean", color: "#5e78bd" },
                { id: "garden", color: "#3f9c8c" },
              ] as const
            ).map((option) => (
              <button
                type="button"
                className={`style-choice style-${option.id} ${style === option.id ? "selected" : ""}`}
                key={option.id}
                onClick={() => setStyle(option.id)}
              >
                <span style={{ background: option.color }} aria-hidden="true" />
                {strings.styleLabels[option.id]}
              </button>
            ))}
          </div>
        </fieldset>
              ...
              <div className="chip-row">
                {MASCOT_KINDS.map((kind) => (
                  <button
                    type="button"
                    key={kind}
                    className={`mascot-chip ${mascot.kind === kind ? "selected" : ""}`}
                    aria-pressed={mascot.kind === kind}
                    onClick={() => setMascot({ ...mascot, kind })}
                  >
                    <Mascot kind={kind} color={mascot.color} size={44} reducedMotion />
                    {strings.mascotKinds[kind]}
                  </button>
                ))}
              </div>
              ...
              <div className="chip-row">
                {MASCOT_COLORS.map((color) => (
                  <button
                    type="button"
                    key={color}
                    className={`mascot-chip ${mascot.color === color ? "selected" : ""}`}
                    aria-pressed={mascot.color === color}
                    onClick={() => setMascot({ ...mascot, color })}
                  >
                    <span
                      className="color-dot"
                      style={{ background: getMascotPalette(color).accent }}
                      aria-hidden="true"
                    />
                    {strings.mascotColors[color]}
                  </button>
                ))}
              </div>
            </div>
        ...
        <fieldset>
          <legend>{strings.quickSettings}</legend>
...
function StoreOverview({
  ...
  const shelves = [
    profile.store.unlockedProducts.slice(0, half),
    profile.store.unlockedProducts.slice(half),
  ];
  return (
    <main id="main-content" tabIndex={-1} className="game-screen">
      <header className="topbar">
        <div>
          <p className="eyebrow">{strings.yourShop}</p>
          <h1>{storeText.name}</h1>
        </div>
        <div className="cash-badge" aria-label={strings.cashBadgeLabel(profile.cash)}>
          {strings.money(profile.cash)}
        </div>
      </header>
      {notice && (
      ...
      <section
        className={`diorama style-${profile.store.style}`}
        style={{ "--store-color": store.color } as React.CSSProperties}
        aria-label={strings.dioramaLabel(storeText.name)}
      >
        <div className="diorama-sky" />
        <div className="block-shelf shelf-one" aria-hidden="true">
          {shelves[0].map((productId) => (
            <ProductArt key={productId} id={productId} size="small" />
          ))}
        </div>
        <div className="block-shelf shelf-two" aria-hidden="true">
          {shelves[1].map((productId) => (
            <ProductArt key={productId} id={productId} size="small" />
          ))}
        </div>
        <div className="counter" />
        <div className="expansion-blocks" aria-hidden="true">
          {profile.store.purchasedProducts.map((productId) => (
            <ProductArt key={productId} id={productId} size="small" />
          ))}
        </div>
        <Mascot
          kind={profile.mascot.kind}
          color={profile.mascot.color}
          size={132}
          reducedMotion={profile.accessibility.reducedMotion}
          className="diorama-mascot"
        />
      </section>
      <section className="store-actions">
        <div>
          <p className="eyebrow">{strings.dayAndChapter(profile.day, profile.chapter)}</p>
          <h2>{strings.ready}</h2>
          <p className="muted">{strings.readyDescription}</p>
          <div className="objective-line">
            <strong>
              {strings.objectiveOptional}: {objectiveText.title}
            </strong>
            <span>{objectiveDone ? strings.completed : objectiveText.description}</span>
          </div>
          <p className="achievement-line">
            {strings.storeAchievements}: <strong>{profile.achievements.length}</strong>
          </p>
        </div>
      ...
      <section className="shop-guide">
        {nextProduct ? (
          <>
            <ProductArt id={nextProduct.id} size="small" platform platformColor={store.color} />
            <div>
              <p className="eyebrow">{strings.nextGoal}</p>
              <p>
                {profile.cash >= nextCost
                  ? strings.canBuyNow(strings.productName(nextProduct), strings.money(nextCost))
                  : strings.missingAmount(
                      strings.money(nextCost - profile.cash),
                      strings.productName(nextProduct),
                    )}
              </p>
            </div>
            <button type="button" className="secondary-button" onClick={onShop}>
              {strings.goShopping}
            </button>
          </>
        ) : (
...
function GameScreen({
  ...
  useEffect(() => {
    ...
    feedbackRef.current?.scrollIntoView({
      behavior: profile.accessibility.reducedMotion ? "auto" : "smooth",
      block: "center",
    });
  }, [
    profile.accessibility.reducedMotion,
    session.errorsForCurrent,
    ...
    return (
      <main id="main-content" tabIndex={-1} className="game-screen">
        <section className="summary-card">
          <p className="eyebrow">{strings.finishedDay}</p>
          <h1>{strings.storeSummary}</h1>
          <p className="summary-total">{strings.money(session.revenue)}</p>
          <p>{strings.summaryText}</p>
    ...
    <main id="main-content" tabIndex={-1} className="game-screen service-screen">
      <header className="service-header">
        <button
          ...
          aria-label={strings.backToShopLabel}
          onClick={onLeave}
        >
          {strings.backToShop}
        </button>
        <span>{strings.customerCounter(session.completedVisits + 1, session.visits.length)}</span>
        <span className="header-money">
          <span className="header-money-item">
            <small>{strings.cashLabel}</small>
            <strong>{strings.money(profile.cash)}</strong>
          </span>
          <span className="header-money-item">
            <small>{strings.todayLabel}</small>
            <strong>{strings.money(session.revenue)}</strong>
          </span>
        </span>
      </header>
      <section className="customer-card">
        <Mascot
          ...
          color={profile.mascot.color}
          size={76}
          mood={session.feedback?.kind === "correct" ? "celebrate" : "idle"}
          reducedMotion={profile.accessibility.reducedMotion}
        />
        <div>
          <p className="eyebrow">{strings.customerArrived(visit.customer.name)}</p>
          <h1>{strings.customerPhrase(visit.customer)}</h1>
          <p className="customer-request">
            {request.before}
            <strong>{request.emphasis}</strong>
            {request.after}
          </p>
        </div>
        <ProductArt id={visit.product.id} size="medium" platform />
      </section>
      {session.phase === "customer" && (
        <section className="question-card intro-card">
          <p>{strings.howMany}</p>
          <button type="button" className="primary-button" onClick={onStartQuestion}>
            {strings.seeAccount}
          </button>
        </section>
      )}
      {session.phase === "product-select" && (
        <section className="question-card">
          <p className="eyebrow">{strings.separateProducts}</p>
          <h2>{strings.quantityQuestion(visit.product)}</h2>
          <div className="product-counter">
            <button
              type="button"
              aria-label={strings.removeProduct}
              onClick={() => onSelectQuantity(session.selectedQuantity - 1)}
              disabled={session.selectedQuantity === 0}
            >
              −
            </button>
            <div
              className="product-pile"
              aria-label={strings.quantityPileLabel(session.selectedQuantity, visit.quantity)}
            >
              {Array.from({ length: session.selectedQuantity }, (_, index) => (
                <ProductArt key={index} id={visit.product.id} size="tiny" />
              ))}
            </div>
            <button
              type="button"
              aria-label={strings.addProduct}
              onClick={() => onSelectQuantity(session.selectedQuantity + 1)}
              disabled={session.selectedQuantity === visit.quantity}
            >
              +
            </button>
          </div>
          <p className="muted">
            {strings.quantityProgress(session.selectedQuantity, visit.quantity)}
          </p>
        </section>
      )}
      {(session.phase === "question" || session.phase === "feedback") && (
        <section className="question-card">
          {/* O desenho é redundante com a frase abaixo: escondê-lo evita 10 anúncios iguais no leitor de tela. */}
          <div className="unit-array" aria-hidden="true">
            {Array.from({ length: visit.quantity }, (_, index) => (
              <span className="unit-item" key={index}>
                <ProductArt id={visit.product.id} size="tiny" />
                <span className="unit-price">{strings.money(visit.product.price)}</span>
              </span>
            ))}
          </div>
          <p className="unit-explain">
            {strings.unitExplain(visit.quantity, visit.product, strings.money(visit.product.price))}
          </p>
          <p className="equation-context">
            {strings.equation(visit.quantity, visit.product.price)}
          </p>
          <h2>{strings.qaTitle}</h2>
          <div className="alternatives" role="group" aria-label={strings.answersLabel}>
            {alternatives.map((alternative) => (
              <button
                type="button"
                key={alternative.value}
                className="answer-button"
                onClick={() => session.phase === "question" && onAnswer(alternative.value)}
                disabled={session.phase === "feedback"}
              >
                {strings.money(alternative.value)}
              </button>
            ))}
          </div>
          {session.phase === "feedback" && session.feedback?.kind === "incorrect" && (
            <div ref={feedbackRef} className="hint-box" role="status" aria-live="polite">
              <strong>{strings.wrongAnswer}</strong>
              <p>{hintText}</p>
              {session.errorsForCurrent >= 4 ? (
                <button
                  type="button"
                  className="secondary-button"
                  onClick={() => onAnswer(visit.fact.answer)}
                >
                  {strings.useFullAnswer}
                </button>
              ) : (
          ...
          {session.phase === "feedback" && session.feedback?.kind === "correct" && (
            <div ref={feedbackRef} className="success-box" role="status" aria-live="polite">
              <strong>{strings.correctAnswer(visit.fact.answer)}</strong>
              <p>{strings.answerAccepted}</p>
              <button type="button" className="primary-button" onClick={onContinue}>
                {session.completedVisits === session.visits.length
                  ? strings.seeCloseout
...
function AchievementsScreen({
  ...
  return (
    <main id="main-content" tabIndex={-1} className="form-screen">
      ...
      <p className="eyebrow">{strings.achievementsEyebrow}</p>
      <h1>{strings.achievements}</h1>
      <p className="muted">{strings.achievementsDescription}</p>
      <div className="product-grid">
        {achievements.map((achievement) => {
          const text = strings.achievementText(achievement);
          return (
            <article
              className={`product-card achievement-card ${achievement.unlocked ? "unlocked" : "locked"}`}
              key={achievement.id}
            >
              ...
              <strong>
                {achievement.unlocked ? strings.achievementUnlocked : strings.achievementLocked}
              </strong>
            </article>
          );
        })}
...
function ShopScreen({
  ...
  onCosmeticPurchase: (cosmeticId: string, cost: number) => void;
}) {
  const store = getStore(profile.store.storeId);
  const purchaseStatus = (cost: number, productName: string) => {
    const canAfford = profile.cash >= cost;
    return {
      canAfford,
      description: strings.purchaseUnavailable(strings.money(cost - profile.cash), productName),
    };
  };
  return (
    <main id="main-content" tabIndex={-1} className="form-screen">
      ...
      <header className="topbar">
        <div>
          <p className="eyebrow">{strings.catalog}</p>
          <h1>{strings.productsNew}</h1>
        </div>
        <div className="cash-badge" aria-label={strings.cashBadgeLabel(profile.cash)}>
          {strings.money(profile.cash)}
        </div>
      ...
      <p className="muted">{strings.catalogDescription}</p>
      <div className="product-grid">
        {store.products.map((product) => {
          const unlocked = profile.store.unlockedProducts.includes(product.id);
          const cost = product.unlockCost ?? 80;
          const productName = strings.productName(product);
          const purchase = purchaseStatus(cost, productName);
          const hintId = `purchase-hint-${product.id}`;
          return (
            <article className={`product-card ${unlocked ? "unlocked" : ""}`} key={product.id}>
              <ProductArt id={product.id} size="medium" platform platformColor={store.color} />
              <h2>{productName}</h2>
              <p>{strings.priceLine(product.price)}</p>
              {unlocked ? (
                ...
                <>
                  ...
                  <button
                    type="button"
                    className={`secondary-button purchase-button ${purchase.canAfford ? "purchase-button--available" : "purchase-button--unavailable"}`}
                    aria-disabled={purchase.canAfford ? undefined : true}
                    aria-describedby={purchase.canAfford ? undefined : hintId}
                    onClick={() => onPurchase(product.id, cost)}
                  >
                    {strings.buyForLabel(cost)}
                  </button>
                </>
              )}
            </article>
      ...
      <h2 className="section-title">{strings.decor}</h2>
      <div className="product-grid">
        {COSMETICS.map((cosmetic) => {
          const owned = profile.store.cosmetics.includes(cosmetic.id);
          const productName = strings.cosmeticName(cosmetic.id, cosmetic.name);
          const purchase = purchaseStatus(cosmetic.cost, productName);
          const hintId = `purchase-hint-${cosmetic.id}`;
          return (
            <article className="product-card" key={cosmetic.id}>
              <ProductArt id={cosmetic.id} size="medium" platform />
              <h2>{productName}</h2>
              <p>{strings.decorDescription}</p>
              {owned ? (
                <strong>{strings.inCollection}</strong>
              ) : (
                <>
                  {!purchase.canAfford && (
                    <span id={hintId} className="sr-only">
                  ...
                  <button
                    ...
                    aria-disabled={purchase.canAfford ? undefined : true}
                    aria-describedby={purchase.canAfford ? undefined : hintId}
                    onClick={() => onCosmeticPurchase(cosmetic.id, cosmetic.cost)}
                  >
                    {strings.buyForLabel(cosmetic.cost)}
                  </button>
                </>
              )}
            </article>
...
function SettingsScreen({
  ...
  profile: PlayerProfile;
  onBack: () => void;
  onSave: (settings: AccessibilitySettings, audio: AudioSettings) => void;
  ...
  return (
    <main id="main-content" tabIndex={-1} className="form-screen">
      ...
      <p className="eyebrow">{strings.settingsEyebrow}</p>
      <h1>{strings.settings}</h1>
      <fieldset>
        <legend>{strings.accessibility}</legend>
        <label className="check-row">
          <input
            type="checkbox"
        ...
        <label className="check-row">
          <input
            ...
            onChange={(event) => setSettings({ ...settings, largeText: event.target.checked })}
          />{" "}
          {strings.largeText}
        </label>
        <label className="check-row">
          <input
            ...
            onChange={(event) => setSettings({ ...settings, highContrast: event.target.checked })}
          />{" "}
          {strings.highContrast}
        </label>
      </fieldset>
      <fieldset>
        <legend>{strings.audioAndNarration}</legend>
        <label className="check-row">
          <input
            type="checkbox"
            checked={audio.effects}
            onChange={(event) => setAudio({ ...audio, effects: event.target.checked })}
          />{" "}
          {strings.audioEffects}
        </label>
        <label className="check-row">
          <input
            type="checkbox"
            checked={audio.narration}
            onChange={(event) => setAudio({ ...audio, narration: event.target.checked })}
          />{" "}
          {strings.narration}
        </label>
      </fieldset>
      <button type="button" className="primary-button" onClick={() => onSave(settings, audio)}>
        {strings.saveSettings}
      </button>
      <section className="danger-zone">
        <h2>{strings.resetTitle}</h2>
        <p className="muted">{strings.resetDescription}</p>
        {confirmingReset ? (
          <div
            className="reset-confirm"
            role="alertdialog"
            aria-label={strings.resetConfirmQuestion}
          >
            <p>
              <strong>{strings.resetConfirmQuestion}</strong>
            </p>
            <button type="button" className="secondary-button" onClick={() => void onReset()}>
